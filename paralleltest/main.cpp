...
bool at_subdomain_border(const int x,const int y,const int z){
    return(x==0 || y==0 || z==0 || x==settings::x_local_grid_size-1 || y==settings::y_local_grid_size || z==settings::z_local_grid_size);
}

bool is_neighbor(const int node_x, const int node_y, const int node_z, const int proc_x_diff, const int proc_y_diff, const int proc_z_diff){
...
double solve_eikonal_quadratic_3d(SubdomainData &subdomain, const int x, const int y, const int z){
    ...
    double temp_res = std::numeric_limits<double>::max();
    ...
    while(num_dir!=0){
        ...
        double c = pow(h_array[0]*min_res_array[0],2)+pow(h_array[1]*min_res_array[1],2)+pow(h_array[2]*min_res_array[2],2)-pow(speed_funct(x,y,z),-2);
        if((pow(b,2)-4*a*c)>=0){
...
void update_neighbors(int subdomain_index, SubdomainData &subdomain, const int x, const int y, const int z){
    ...
    for (std::size_t i = 0; i < neighbors.size(); i += 3){
        ...
        if(subdomain.status_array[neighbor_index]!='3' ){
            ...
            for (int z = 0; z < settings::z_local_grid_size; ++z) {
                ...
                if(at_subdomain_border(x,y,z) && (curr_node_status == '1' || curr_node_status == '4')) {
                    ...
                    for (std::size_t i = 0; i < neighbors.size(); i += 3) {
                        ...
                        if (is_neighbor(x, y, z, x_index - neighbors[i], y_index - neighbors[i + 1],z_index - neighbors[i + 2])) {
                            int process_index_in_neighbor = get_index(neighbors[i], neighbors[i + 1], neighbors[i + 2],
                                                                      x_index, y_index, z_index);
                            ExchangeData data{x, y, z, subdomain.weight_array[local_arr_index(x, y, z)]};
...
void integrate_overlapping_data(const int subdomain_index, double bound_band, SubdomainData subdomain_array[settings::total_num_processes], std::vector<std::vector<ExchangeData>> &exchange_vector) {
    ...
    for(int i=0; i<exchange_vector.size();++i){
        for(int j=0; j < exchange_vector[i].size();++j){
            ...
            int temp_index = local_arr_index(temp_data.x, temp_data.y, temp_data.z);
            ...
            if(temp_val<subdomain.weight_array[temp_index]) {
                ...
                int heap_index = subdomain.h.get_heap_index(temp_data.x, temp_data.y, temp_data.z);
                if(heap_index==-1){
                    subdomain.h.insertKey(WeightedPoint{temp_data.x, temp_data.y, temp_data.z, temp_val});
                }
                else {
...
void march_narrow_band(const int subdomain_index, SubdomainData subdomain_array[settings::total_num_processes], double bound_band) {
    ...
    while(true){
        ...
        if(subdomain.weight_array[curr_index]> bound_band){
            break;
        }
...
int main() {
    ...
    while(true){
        ...
        if(*min_val_global > width_band - eps && *count_global ==0){
            break;
        }
    ...
    for(int x =0; x<settings::x_global_grid_size;++x){
        for(int y =0; y<settings::y_global_grid_size;++y){
            for(int z =0; z<settings::z_global_grid_size;++z){
                ...
                weight_array[global_arr_index(x,y,z)]=subdomain_array[process_index(process_xid, process_yid,process_zid)].weight_array[x_local_coord, y_local_coord, z_local_coord];
