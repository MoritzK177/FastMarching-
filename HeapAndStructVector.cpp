...
#include <array>
#include <vector>
int arr_index(int x, int y, int z);//forward declaration
...
int MinHeap::get_heap_index(int x, int y, int z)
{
    return table[arr_index(x,y,z)];
}

// Inserts a new point 'point'
void MinHeap::insertKey(WeightedPoint point)
{
    ...
    //harr[i] = point;
    table[arr_index(point.m_x,point.m_y,point.m_z)]=i;
...
WeightedPoint MinHeap::extractMin()
{
    if (heap_size <= 0)
    {
        ...
        //WeightedPoint c{-1, -1, -1, 42069};
        //return c;
    }
    if (heap_size == 1)
    {
        heap_size--;
        table[arr_index(harr[0].m_x,harr[0].m_y,harr[0].m_z)]=-1;
        return harr[0];
    }
    ...
    heap_size--;
    MinHeapify(0);
    table[arr_index(root.m_x,root.m_y,root.m_z)]=-1;
...
{
    int temp_index=table[arr_index((*x).m_x, (*x).m_y, (*x).m_z)];
    table[arr_index((*x).m_x, (*x).m_y, (*x).m_z)]=table[arr_index((*y).m_x, (*y).m_y, (*y).m_z)];
    table[arr_index((*y).m_x, (*y).m_y, (*y).m_z)]=temp_index;
